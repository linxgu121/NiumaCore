...
## 场景挂载与 Inspector 配置
NiumaCore 是公共协议包，不需要在 Unity 场景中挂任何脚本。

| 内容 | 说明 |
| --- | --- |
| 是否需要 GameObject | 不需要 |
| 是否需要 Inspector 配置 | 不需要 |
| 谁引用它 | 其他模块的 Controller、Service、Bridge、SaveAdapter |
| 不配置会怎样 | Core 本身没有运行时对象，不存在“不配置”问题 |

使用约定：

- 不要把业务模块脚本放进 Core。
- 不要在 Core 中引用 Unity 场景对象。
- 进入 Core 的 DTO、枚举、接口要长期稳定，特别是会进入存档、网络协议或 GameContext 的类型。
